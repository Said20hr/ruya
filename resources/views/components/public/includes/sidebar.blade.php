<div class="dark:bg-dark w-fit">
    <!-- Mobile & Responsive Screen -->
    <header class="xl:hidden
    {{ config('app.locale') == 'ar' ? "border-l" : 'border-r' }}
    fixed left-O top-0 xl:max-w-52 w-3/5 py-4 xl:px-6 px-4 h-full z-50 transition-all ease-soft-in-out duration-600 dark:bg-gray-900 bg-white border-slate-100 dark:border-slate-900"
            x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0 {{ config('app.locale') == 'ar' ? "translate-x-100" : '-translate-x-100' }} "
            x-transition:enter-end="opacity-100 translate-x-0"
            x-transition:leave="transition ease-in duration-300"
            x-transition:leave-start="opacity-100 translate-x-0"
            x-transition:leave-end="opacity-0 {{ config('app.locale') == 'ar' ? "translate-x-100" : '-translate-x-100' }} "
        ...
        <nav class="sidebar " >
            <div class="navbar-collapse z-50" id="navbar-collapse">
                ...
                <ul class="xl:pt-4 pt-16 px-0">
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('visuals')}}" title="Visuals" class="text-base {{ Route::currentRouteName() == 'visuals' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100' }}"><i class="fa fa-image mx-2"></i>{{__('Visuals'}} </a></li>
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('motion')}}" title="Motions" class="text-base {{ Route::currentRouteName() == 'motion' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100' }}"><i class="fa fa-spinner mx-2"></i>{{__('Motions'}} </a></li>
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('dev')}}" title="Development" class="text-base {{ Route::currentRouteName() == 'dev' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100' }}"><i class="fa fa-code mx-2"></i>{{__('Development'}} </a></li>
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('animation')}}" title="Development" class="text-base {{ Route::currentRouteName() == 'animation' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100' }}"><i class="fa fa-cube mx-2"></i>{{__('3D & Rendering'}}</a></li>
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('services')}}" class="text-base dark:text-gray-100 {{ Route::currentRouteName() == 'services' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100'  }}"><i class="fa fa-handshake-o mx-2"></i>{{__('Services'}} </a></li>
                    <li class="py-3 {{ config('app.locale') == 'ar' ? "pl-2" : 'pr-2' }} hover:bg-gray-200 dark:hover:bg-gray-700"><a href="{{route('contact')}}" title="Contact" class="text-base {{ Route::currentRouteName() == 'contact' ? 'font-bold text-black dark:text-white' : 'text-gray-800 dark:text-gray-100' }}" ><i class="fa fa-envelope mx-2"></i>{{__('Contact Us'}} </a></li>
                </ul>
                <div class="py-2 px-1 text-black" x-show="OpenNavResponsive" x-cloak="">
                    <div class="relative inline-block w-16 select-none" x-data="{ toggle: darkMode }"
                        ...
                        <div :class="{' {{ config('app.locale') == 'ar' ? "-translate-x-full" : 'translate-x-full' }}': toggle}"
                             class="absolute z-10 border-gray-800 block w-8 h-6 px-1 rounded-full bg-white border-4 cursor-pointer transition duration-500 transform"></div>
                        <div class="overflow-hidden bg-gray-800 h-6 w-full rounded-full cursor-pointer flex justify-between items-center p-px p-0.5">
                            <!-- feathericons -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6  text-yellow-400 fill-current" viewBox="0 0 24 24" >
                                <path d="M0 0h24v24H0z" fill="none" />
                                <path d="M6.76 4.84l-1.8-1.79-1.41 1.41 1.79 1.79 1.42-1.41zM4 10.5H1v2h3v-2zm9-9.95h-2V3.5h2V.55zm7.45 3.91l-1.41-1.41-1.79 1.79 1.41 1.41 1.79-1.79zm-3.21 13.7l1.79 1.8 1.41-1.41-1.8-1.79-1.4 1.4zM20 10.5v2h3v-2h-3zm-8-5c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6-2.69-6-6-6zm-1 16.95h2V19.5h-2v2.95zm-7.45-3.91l1.41 1.41 1.79-1.8-1.41-1.41-1.79 1.8z" />
                            </svg>
                            <!-- material icons -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6  text-blue-300 fill-current" viewBox="0 0 24 24"
                ...
                <div x-data="{'dropdown' : false,language : 'Français' }" class="relative my-2 dark:text-white text-gray-700 lg:text-xs 2xl:text-base xl:text-base text-base">
                    <button x-cloak="" x-on:click="dropdown = !dropdown" type="button" class="mx-2 xl:py-2 py-1 xl:px-3 flex items-center">
            ...
            <div class="fixed z-0 bottom-2">
                ...
                <p class="xl:text-xs text-sm dark:text-white">Copyright &copy; {{ date('Y') }} Ruya Studio <br> All Rights Reserved</p>
            </div>
        </nav>
    </header>
    <!-- Large Screen -->
